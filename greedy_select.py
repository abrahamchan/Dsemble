...
def print_best_ensemble(encoded_str, ens_stat_list, dataset, fault_type, elapsed_time, symmetric):
    ...
    if symmetric:
        desemble_logfilename = "./output/" + dataset + "_" + fault_type + "_greedy_select"
    else:
        desemble_logfilename = "./output/" + dataset + "_" + fault_type + "_asymmetric_greedy_select"
...
def main():
    ...
    for fault_amt in fault_amt_arr:
        ...
        for model_i in range(N):
            if model_i in fixed_positions:
                continue

            model_name = model_list[model_i]

            if not config_exists(dataset, model_name, final_fault, symmetric, div_op):
                print("Training arch: ", model_name)
                arch(dataset, model_name, final_fault, symmetric, num_epochs, batch_size)

            pred_filename = get_pred_filename(dataset, model_name, final_fault, symmetric, div_op)
            curr_df = pd.read_json(pred_filename)
            curr_df.columns = ["model"]
            curr_df["ground"] = ground_labels

            # Calculate fitness score
            corr = curr_df[curr_df["model"]==curr_df["ground"]].shape[0]
            total = curr_df.shape[0]
            accuracy = corr / total
            fitness = accuracy
            ...
            if fitness > best_score:
                best_score = fitness
                best_model_position = model_i

        fixed_positions.append(best_model_position)
        encoded_arr[best_model_position] = 1
